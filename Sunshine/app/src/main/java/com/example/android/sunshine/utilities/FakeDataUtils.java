...
public class FakeDataUtils {
    ...
    private static ContentValues createTestWeatherContentValues(long date) {
        ...
        testWeatherValues.put(WeatherEntry.COLUMN_WIND_SPEED, Math.random() * 100);
        testWeatherValues.put(WeatherEntry.COLUMN_WEATHER_ID, weather_ID[(int) ((Math.random() * 10) / 5)]);
    ...
    public static void insertFakeData(Context context) {
        ...
        for (int i = 0; i < 7; i++) {
            fakeValues.add(createTestWeatherContentValues(today +
                    TimeUnit.DAYS.toMillis(i)));

        }
        context.getContentResolver().bulkInsert(WeatherEntry.CONTENT_URI, fakeValues.toArray(new ContentValues[7]));
    }
